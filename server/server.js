...
app.post('/api/auth/verify-security', async (req, res) => {
  try {
    ...
    if (!email || !answers || answers.length !== 2) {
      return res.status(400).json({ error: 'Email and 2 security answers are required' });
    }
...
app.post('/api/vault', authenticateToken, async (req, res) => {
  try {
    ...
    if (!encryptedData || !clientSalt) {
      return res.status(400).json({ error: 'Encrypted data and client salt are required' });
    }
