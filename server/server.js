...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (!email || !password || !securityQuestions || !salt)
...
app.get('/api/vault', authenticateToken, async (req, res) => {
  try {
    ...
    if (!vault) {
      return res.json({ message: 'No vault found', encryptedData: null, clientSalt: null });
    }
...
app.post('/api/vault', authenticateToken, async (req, res) => {
  try {
    ...
    if (!encryptedData || !clientSalt) {
      return res.status(400).json({ error: 'Encrypted data and client salt required' });
    }
    ...
    if (vault) {
      ...
      vault = new Vault({ userId: req.user._id, encryptedData, clientSalt });
    }
    await vault.save();
