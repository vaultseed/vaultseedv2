...
function App() {
  ...
  // Vault load helper: tries server first, fallback to local backup; will push local to server if server empty
    ...
    try {
      ...
      if (vaultAPI && typeof vaultAPI.get === 'function') {
        try {
          ...
          // serverVault expected shape: { data: string OR encryptedData, salt OR clientSalt }
          const serverData = serverVault?.data ?? serverVault?.encryptedData ?? null;
          const serverSalt = serverVault?.salt ?? serverVault?.clientSalt ?? null;
          if (serverData && serverSalt) {
            const key = await getKey(password, serverSalt);
            const decrypted = await decryptData(key, serverData);
            if (decrypted) {
              setVaultData(JSON.parse(decrypted));
              localStorage.setItem('vaultseed_salt', serverSalt);
              return serverSalt;
            } else {
      ...
      if (local) {
        try {
          ...
          if (decrypted) {
            ...
            // if server doesn't have vault, push local
            try {
              const serverCheck: any = await vaultAPI.get();
              if (!serverCheck || !(serverCheck.data || serverCheck.encryptedData)) {
          ...
          console.warn('Failed to decrypt local vault:', error);
          showTooltip('Unable to decrypt local vault. Check your password.', 'error');
        }
      }
  ...
  const handleLogout = () => {
    try {
      // remove local cached vault on logout (local backup only)
      if (email) {
        try { localStorage.removeItem(`vaultseed_${btoa(email)}`); } catch {}
      }
    } catch (e) {
      // ignore
    }
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-4 transition-colors duration-300 ${
      ...
      <div className="w-full max-w-7xl">
        {/* Landing / Auth UI (same as above) */}
        {/* Reuse the form UI above for consistency */}
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 mb-12">
          <div className="lg:w-1/2">
            {/* Branding */}
            <div className="mb-8">
